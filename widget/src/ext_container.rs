...
use crate::core::{layout, ExtBorder, ExtPath, ExtPolygon,ExtBackground, ExtShadow};
...
where
    ...
    Renderer: core::Renderer,
{
    /// Creates a [`ExtContainer`] with the given content.
    pub fn new(
    ...
    /// Sets the [`widget::Id`] of the [`ExtContainer`].
    pub fn id(mut self, id: impl Into<widget::Id>) -> Self {
        self.id = Some(id.into());
        self
    }

    /// Sets the [`Padding`] of the [`ExtContainer`].
    pub fn padding<P: Into<Padding>>(mut self, padding: P) -> Self {
        self.padding = padding.into();
        self
    }

    /// Sets the width of the [`ExtContainer`].
    pub fn width(mut self, width: impl Into<Length>) -> Self {
        self.width = width.into();
        self
    }

    /// Sets the height of the [`ExtContainer`].
    pub fn height(mut self, height: impl Into<Length>) -> Self {
        self.height = height.into();
        self
    }

    /// Sets the maximum width of the [`ExtContainer`].
    pub fn max_width(mut self, max_width: impl Into<Pixels>) -> Self {
        self.max_width = max_width.into().0;
        self
    }

    /// Sets the maximum height of the [`ExtContainer`].
    pub fn max_height(mut self, max_height: impl Into<Pixels>) -> Self {
        self.max_height = max_height.into().0;
        self
    }

    /// Sets the width of the [`ExtContainer`] and centers its contents horizontally.
    pub fn center_x(self, width: impl Into<Length>) -> Self {
        self.width(width).align_x(alignment::Horizontal::Center)
    }

    /// Sets the height of the [`ExtContainer`] and centers its contents vertically.
    ...
    /// Sets the content alignment for the vertical axis of the [`ExtContainer`].
    pub fn align_y(
        mut self,
    ...
    /// Sets whether the contents of the [`ExtContainer`] should be clipped on
    /// overflow.
    ...
    /// Sets the style of the [`ExtContainer`].
    #[must_use]
    ...
    /// Sets the style class of the [`ExtContainer`].
    #[must_use]
...
for ExtContainer<'_, Message, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: core::Renderer,
{
...
impl<'a, Message, Theme, Renderer> From<ExtContainer<'a, Message, Theme, Renderer>>
...
{
    fn from(
        container: ExtContainer<'a, Message, Theme, Renderer>,
    ) -> Element<'a, Message, Theme, Renderer> {
        Element::new(container)
    }
}

/// Computes the layout of a [`ExtContainer`].
pub fn layout(
    limits: &layout::Limits,
    width: Length,
    height: Length,
...
/// Draws the background of a [`ExtContainer`] given its [`Style`] and its `bounds`.
...
pub struct Style {
    ...
    /// The [`ExtBackground`] of the container.
    pub background: Option<ExtBackground>,
    /// The [`ExtBorder`] of the container.
    pub border: ExtBorder,
    /// The [`Shadow`] of the container.
    pub shadow: ExtShadow,
    /// The ['ExtPath'] of the container
    pub path: ExtPath,
...
impl Default for Style {
    fn default() -> Self {
        Self {
            text_color: None,
            background: None,
            border: ExtBorder::default(),
            shadow: ExtShadow::default(),
            path: ExtPath::Quad(Radius::new(0.)),
...
impl Style {
    ...
    pub fn border(self, border: impl Into<ExtBorder>) -> Self {
        Self {
            border: border.into(),
            ..self
        }
    ...
    pub fn background(self, background: impl Into<ExtBackground>) -> Self {
        Self {
    ...
    pub fn shadow(self, shadow: impl Into<ExtShadow>) -> Self {
        Self {
            shadow: shadow.into(),
            ..self
        }
...
/// The theme catalog of a [`ExtContainer`].
pub trait Catalog {
...
/// A styling function for a [`ExtContainer`].
pub type StyleFn<'a, Theme> = Box<dyn Fn(&Theme) -> Style + 'a>;

impl<Theme> From<Style> for StyleFn<'_, Theme> {
    fn from(style: Style) -> Self {
        Box::new(move |_theme| style.clone())
    }
}
...
/// A transparent [`ExtContainer`].
pub fn transparent<Theme>(_theme: &Theme) -> Style {
    Style::default()
}

/// A [`ExtContainer`] with the given [`Background`].
pub fn background(background: impl Into<ExtBackground>) -> Style {
    Style::default().background(background)
}

/// A rounded [`ExtContainer`] with a background.
pub fn ext_rounded_box(theme: &Theme) -> Style {
    let palette = theme.extended_palette();

    Style {
        background: Some(palette.background.weak.color.into()),
        text_color: Some(palette.background.weak.text),
        border: ExtBorder::rounded(2),
        ..Style::default()
    }
}

/// A bordered [`ExtContainer`] with a background.
pub fn ext_bordered_box(theme: &Theme) -> Style {
    let palette = theme.extended_palette();
...
/// A [`ExtContainer`] with a dark background and white text.
...
/// A [`ExtContainer`] with a primary background color.
...
/// A [`ExtContainer`] with a secondary background color.
...
/// A [`ExtContainer`] with a success background color.
...
/// A [`ExtContainer`] with a danger background color.
